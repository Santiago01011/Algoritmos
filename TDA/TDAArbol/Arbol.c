...
int grabarArchivoDesdeArbol(tArbol *p, const char *path, size_t tam){
    FILE *pf = fopen(path, "wb");
    if(!pf)
        return 0;
    grabarArchivoDesdeArbolRec(p, pf, tam);
    fclose(pf);
    return 1;
}

void fwriteA(void *info, size_t tamInfo, unsigned n, void *pf){
    fwrite(info, tamInfo, 1, (FILE*)pf);
}

void grabarArchivoDesdeArbolRec(tArbol *p, FILE *pf, size_t tam){
    if(!*p)
        return;
    recInorden(p, (AccionAvz)fwriteA, pf);
}
